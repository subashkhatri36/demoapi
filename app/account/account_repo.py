...
from app.company.company_models import Company
from app.account.account_models import User
...
def create(request: account_schemas.User,db:Session):
    checkuser=db.query(User).filter(User.email == request.email).first()
    if checkuser:
        response={'detail':{'status':'false','data': "Email Already Exist"}}
    else:
        ...
        db.commit()
        db.refresh(new_user)
        filterdata=db.query(Company).filter(Company.user_id == new_user.id).first()
        if filterdata:
            userlist=filterdata
        else:
            userlist=[]
        userresponse={'email':new_user.email,'id':new_user.id,'fullname':new_user.fullname,'profile':new_user.profile,'last_login':new_user.last_login,'is_superuser':new_user.is_superuser,'is_staff':new_user.is_staff,'date_joined':new_user.date_joined,"is_active":new_user.is_active,"company":userlist}
